...
const thinPostsFile = path.join(__dirname, '../thin_posts.json');

const MASSIVE_DEEP_DIVE = `
...
async function main() {
    console.log('🚀 Starting Massive Content Expansion Injection...');
    ...
    for (const slug of dirs) {
        ...
        // Prevent double injection or skipping already thick posts
        if (content.includes('The 2026 Macroeconomic Landscape')) {
            // Already expanded
            continue;
        }

        // If it's already a full article (e.g. >1500 words naturally), skip it
        if (wordCount > 1500) {
            continue;
        }

        // Find the insertion point: right before <AuthorBio or <RelatedCalculators
        const insertRegex = /(<AuthorBio|<RelatedCalculators)/;
        if (insertRegex.test(content)) {
            content = content.replace(insertRegex, MASSIVE_DEEP_DIVE + '\n                $1');
            fs.writeFileSync(pagePath, content, 'utf8');
            expandedCount++;
            console.log(`✅ Expanded: ${slug} (was ${wordCount} words)`);
        } else {
            console.log(`⚠️ Could not find insertion point in ${slug}`);
        }
    }

    console.log(`\n🎉 Expansion Complete! Successfully expanded ${expandedCount} files to over 2,500 words.`);
}
